...
func (r *domainResolver) init(ctx context.Context) (err error) { //nolint:gocyclo
	...
	errF := func(_ any, e error) func(ignoreNotFound bool) {
		return func(ignoreNotFound bool) {
			if errors.Is(e, conf.ErrNotFound) && ignoreNotFound {
				return
			}
			if e != nil {
				errs = append(errs, e)
	...
	errF(conf.DnsNameservers.Value(ctx,
		conf.DnsNameservers.OptSink(func(ips []conf.IP) error {
			var o dns.WithNameservers
			linq.From(ips).
				Select(func(i any) any {
					return i.(conf.IP).String()
				}).ToSlice(&o)
			if len(o) == 0 {
	...
	errF(conf.DnsProto.Value(ctx,
		conf.DnsProto.OptSink(func(s string) error {
			switch s {
			case "tcp":
	...
	errF(conf.DnsPort.Value(ctx,
		conf.DnsPort.OptSink(func(u uint16) error {
			if u == 0 {
				return errPortZero
			}
	...
	errF(conf.DnsRetriesTmo.Value(ctx,
		conf.DnsRetriesTmo.OptSink(func(d time.Duration) error {
			if d > 0 {
	...
	errF(conf.DnsRetries.Value(ctx,
		conf.DnsRetries.OptSink(func(u uint8) error {
			if u > 0 {
				if bkf == nil {
					bkf = &bf.ZeroBackOff{}
				}
	...
	errF(conf.DnsDialDuration.Value(ctx,
		conf.DnsDialDuration.OptSink(func(d time.Duration) error {
			if d > 0 {
	...
	errF(conf.DnsWriteDuration.Value(ctx,
		conf.DnsWriteDuration.OptSink(func(d time.Duration) error {
			if d > 0 {
	...
	errF(conf.DnsReadDuration.Value(ctx,
		conf.DnsReadDuration.OptSink(func(d time.Duration) error {
			if d > 0 {
