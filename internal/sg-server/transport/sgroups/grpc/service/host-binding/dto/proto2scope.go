...
func init() {
	dto.Register[*pb.HostBindingReq_Watch, filter.Scope](watchReqToScope)
}
...
type proto2scopeVariants interface {
	*dto.Pair[*pb.HostBindingReq_Watch, filter.Scope]
	Convert() error
}

func watchReqToScope(src *pb.HostBindingReq_Watch) (dest filter.Scope, err error) {
	pbSel := src.GetSelectors()
	sels := misc.Tern(len(pbSel) > 0, make([]domain.HostBindingSelector, len(pbSel)), nil)
	...
	dest = scopes.ScopedAnd{
		...
		R: scopes.ByHostBindingSelectors(sels...),
	}
	return dest, nil
}
