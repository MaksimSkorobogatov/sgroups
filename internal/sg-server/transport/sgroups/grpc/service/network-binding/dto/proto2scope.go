...
func init() {
	dto.Register[*pb.NetworkBindingReq_Watch, filter.Scope](watchReqToScope)
}
...
type proto2scopeVariants interface {
	*dto.Pair[*pb.NetworkBindingReq_Watch, filter.Scope]
	Convert() error
}

func watchReqToScope(src *pb.NetworkBindingReq_Watch) (dest filter.Scope, err error) {
	pbSel := src.GetSelectors()
	sels := misc.Tern(len(pbSel) > 0, make([]domain.NetworkBindingSelector, len(pbSel)), nil)
	...
	dest = scopes.ScopedAnd{
		...
		R: scopes.ByNetworkBindingSelectors(sels...),
	}
	return dest, nil
}
