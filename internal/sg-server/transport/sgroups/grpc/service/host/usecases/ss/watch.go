...
type (
	// WatchSS - use case for watching socket statistics
	WatchSS struct {
		Rep                 repository.Repository
		AgentClientProvider agent.ClientProvider
		OutStream           grpc.ServerStreamingServer[sgv1.HostResp_SocketStatistics_Watch]
		sendMu              sync.Mutex
	}
)

// Perform - performs use case
func (uc *WatchSS) Perform(ctx context.Context, req *sgv1.HostReq_SocketStatistics_Watch) (err error) {
	...
	_ = parallel.ExecAbstract(len(scopes), int32(len(scopes))-1, func(i int) error { //nolint:gosec
		errs[i] = uc.watchAgentStats(ctx, scopes[i])
		return nil
	})
...
func (uc *WatchSS) watchAgentStats(ctx context.Context, sc hostScope) (err error) {
	defer func() {
	...
	stream, e := agentClient.WatchSocketStatistics(ctx, &agentv1.SocketStatReq_Watch{Selectors: sc.filters})
	if e != nil {
		return errors.WithMessagef(e, "open watch stream for %s", sc)
	}

	var resp *agentv1.SocketStatResp_Watch
	for err == nil {
		...
		uc.sendMu.Lock()
		err = uc.OutStream.Send(&sgv1.HostResp_SocketStatistics_Watch{
			Hosts: []*sgv1.HostResp_SocketStatistics_Host{
				{
