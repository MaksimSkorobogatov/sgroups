...
// Watch - watch for changes in address groups
func (srv *addressGroupService) Watch(req *sgv1.AddressGroupReq_Watch, stream grpc.ServerStreamingServer[sgv1.AddressGroupResp_Watch]) (err error) {
	defer func() {
	...
	if err = rd.WatchAddressGroups(ctx, scope, func(ne domain.AddressGroupEvent) error {
		var msg *sgv1.AddressGroupResp_Watch
		select {
		case <-ctx.Done():
			return ctx.Err()
		default:
