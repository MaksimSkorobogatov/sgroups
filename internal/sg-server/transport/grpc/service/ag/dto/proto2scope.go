...
func init() {
	dto.Register[*pb.AddressGroupReq_Watch, filter.Scope](watchReqToScope)
}
...
type proto2scopeVariants interface {
	*dto.Pair[*pb.AddressGroupReq_Watch, filter.Scope]
	Convert() error
}

func watchReqToScope(src *pb.AddressGroupReq_Watch) (dest filter.Scope, err error) {
