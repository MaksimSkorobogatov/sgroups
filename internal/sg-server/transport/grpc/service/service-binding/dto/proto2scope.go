...
func init() {
	dto.Register[*pb.ServiceBindingReq_Watch, filter.Scope](watchReqToScope)
}
...
type proto2scopeVariants interface {
	*dto.Pair[*pb.ServiceBindingReq_Watch, filter.Scope]
	Convert() error
}

func watchReqToScope(src *pb.ServiceBindingReq_Watch) (dest filter.Scope, err error) {
	pbSel := src.GetSelectors()
	sels := misc.Tern(len(pbSel) > 0, make([]domain.ServiceBindingSelector, len(pbSel)), nil)
	...
	dest = scopes.ScopedAnd{
		...
		R: scopes.ByServiceBindingSelectors(sels...),
	}
	return dest, nil
}
