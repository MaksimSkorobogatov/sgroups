...
// Watch - watch for changes in networks
func (srv *networkService) Watch(req *sgv1.NetworkReq_Watch, stream grpc.ServerStreamingServer[sgv1.NetworkResp_Watch]) (err error) {
	defer func() {
	...
	if err = rd.WatchNetworks(ctx, scope, func(ne domain.NetworkEvent) error {
		var msg *sgv1.NetworkResp_Watch
		select {
		case <-ctx.Done():
			return ctx.Err()
		default:
