...
-- Index for vu_service.refs SubPlan: "where sb.service = svc.id"
create index if not exists service_binding_service_idx
  on sgroups.tbl_service_binding (service);

-- list_services with fast-path for simple selectors (name/namespace only).
-- Avoids materializing vu_service.refs for all rows when the planner
-- can't push the predicate through match_res_selectors().
...
begin
  if selectors is null or cardinality(selectors) = 0 then
    return query select * from sgroups.vu_service;
    return;
  end if;
...
-- Index for vu_host.refs SubPlan: "where hb.host = h.id"
create index if not exists host_binding_host_idx
  on sgroups.tbl_host_binding (host);
